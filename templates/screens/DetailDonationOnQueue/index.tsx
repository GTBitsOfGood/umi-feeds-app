...
function DetailDonationOnQueue(props: {donation: DonationForm}) {

  //const route = useRoute<RouteProp<ParamList, 'DetailDonationOnQueue'>>();
  const { donation } = props;

  let buildingNumberStr = '';
  if (typeof donation.pickupAddress.buildingNumber !== 'undefined') {
    buildingNumberStr = `#${donation.pickupAddress.buildingNumber}`;
  }

  const pickupStartTimeDate = new Date(donation.pickupStartTime);
  ...
  const pickupEndTimeDate = new Date(donation.pickupEndTime);
  ...
  for (let i = 0; i <donation.donationDishes.length; i += 1) {
    ...
    donationDish.push(
      <View key={donation.donationDishes[i].dishID}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <Text>{dishName}</Text>
          <Text>{donation.donationDishes[i].quantity}</Text>
        </View>
        <View style={{ width: '100%', borderTopColor: '#E6E6E6', borderTopWidth: 1, marginVertical: 16 }} />
      </View>
    );
    donationTotalCost += parseInt(donation.donationDishes[i].cost ?? "0")
  }
  ...
  const statusLabel = () => {
    if (donation.status === 'Pending') {
      return (
        ...
        >
          <Text style={{ fontSize: 11, color: '#5D5D5D', fontWeight: 'bold' }}>Set Address</Text>
        </Pressable>
      );
    } else if (donation.status === 'Unclaim') {
      return (
        <Pressable style={{
          width: '18%',
          borderRadius: 4,
        ...
        >
          <Text style={{ fontSize: 10, color: '#007FA7', fontWeight: 'bold' }}>Claim this Donation</Text>
        </Pressable>
      );
      ...
      return (
        ...
        >
          <Text style={{ fontSize: 11, color: '#00883F', fontWeight: 'bold' }}>Complete Donation</Text>
        </Pressable>
      );
    } else {
      return (
        ...
        onPress = {() => console.log("delievery more like smellivery")}
        >
          <Text style={{ fontSize: 11, color: '#FFFFFF', fontWeight: 'bold' }}>Set as Delivered</Text>
        ...
        <View style={styles.container}>
          <View style={{ width: '100%', justifyContent: 'space-between', marginBottom: 20 }}>
            ...
            <Text style={{ fontSize: 21, fontWeight: '500', marginVertical: 24 }}>
              Status: <Text style={{ fontSize: 21, fontWeight: '400', marginVertical: 24 }}>{donation.status}</Text>
            </Text>
            {statusLabel()}
            ...
            <Text style={styles.details}>
              {donation.pickupAddress.streetAddress} {buildingNumberStr}{'\n'}
            ...
            <Text style={styles.details}>{donation.pickupInstructions}</Text>
          </View>
